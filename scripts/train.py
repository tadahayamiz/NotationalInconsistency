# train.py (normalized)
import os, sys, argparse, time, yaml, random, math
...
from notate.training import (
    ...
    hook_type2class, get_hook, AlarmHook
)
...
# utilities
# -------------------------
def to_upper(x: Union[str, None]) -> Union[str, None]:
    return x.upper() if isinstance(x, str) else x

def normalize_loglevels(cfg: ADict) -> None:
    if "logging" in cfg:
        if "stream_level" in cfg.logging:
            cfg.logging.stream_level = to_upper(cfg.logging.stream_level)
        if "file_level" in cfg.logging:
            cfg.logging.file_level = to_upper(cfg.logging.file_level)
...
def build_logger(result_dir: str, cfg: ADict):
    ...
    stream_level = cfg.logging.get("stream_level", "INFO")
    file_level = cfg.logging.get("file_level", "DEBUG")
    logger = default_logger(filename=log_path, stream_level=stream_level, file_level=file_level, logger_name=None)
...
def main():
    ...
    # --- variables expansion ($TIMESTAMP 等) ※明示の変数名を足したい時はここで
    vars_dict = {"$TIMESTAMP": timestamp()}
    cfg = ADict(subs_vars(cfg, vars_dict))

    # --- logging level normalization
    normalize_loglevels(cfg)
    ...
    # --- model
    # 必須: cfg.model.modules（dict）。任意: use_modules / omit_modules / init / seed
    mcfg = cfg.get("model", {})
    ...
    # --- optimizer
    # 必須: cfg.optimizer.type。一般的には lr, weight_decay など
    ...
    # --- metric（あれば）
    metric = None
    mtr = cfg.get("metric", None)
    if mtr and "type" in mtr:
        metric = get_metric(type=mtr["type"], **{k: v for k, v in mtr.items() if k != "type"})
        logger.info(f"[metric] {mtr}")
    ...
    # --- process（実装依存）
    # 典型例: proc(batch, model, optimizer, device, logger, metric=metric) -> dict(loss=..., ...)
    # 実装と引数名に差があれば、この1行の呼び出しだけ合わせればOK。
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            out = process(batch=batch, model=model, optimizer=optimizer, device=device, logger=logger, metric=metric)
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                ...
                for vbatch in val_loader:
                    vcnt += 1
                    _ = process(batch=vbatch, model=model, optimizer=None, device=device, logger=logger, metric=metric)
                    if vcnt >= vsteps:
                        break
