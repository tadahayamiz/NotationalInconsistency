...
def main(config, type, model, params, objective, n_trial,
    ...
    log_file='debug', log_stream='info', log_optuna='warning', ):
    """
    ...
    ## training data
    input_train = get_dataset(logger=logger, name='input_train', dfs=dfs, **data.train.input).array
    target_train = get_dataset(logger=logger, name='target_train', dfs=dfs, **data.train.target).array
    if data.train.mask:
        mask = get_dataset(logger=logger, name='mask_train', dfs=dfs, **data.train.mask).array
        input_train = input_train[mask]
        target_train = target_train[mask]
    
    ## validation data
    if 'val' in data:
        input_val = get_dataset(logger=logger, name='input_val', dfs=dfs, **data.val.input).array
        target_val = get_dataset(logger=logger, name='target_val', dfs=dfs, **data.val.target).array
        if data.val.mask:
            mask = get_dataset(logger=logger, name='mask_val', dfs=dfs, **data.val.mask).array
    ...
    if os.path.exists(path_model) and \
        ('test' not in data or \
        os.path.exists(path_df_pred) and  os.path.exists(path_score)):
        ...
        print(f"training: {input.shape}, {target.shape}")
        model.fit(input, target)
        with open(path_model, 'wb') as f:
            pickle.dump(model, f)
        if 'test' in data:
            input_test = get_dataset(logger=logger, name='target_test', dfs=dfs, **data.test.input).array
            pred_test = model.predict(input_test)
            if data.test.mask:
                mask = get_dataset(logger=logger, name='mask_test', dfs=dfs, **data.test.mask).array
                mu_pred = np.mean(pred_test[mask])
                pred_test[~mask] = mu_pred
            print(f"test: {pred_test.shape}")

            pd.DataFrame({'pred': pred_test}).to_csv(path_df_pred, index=False)
            
            if 'target' in data.test:
                target_test = get_dataset(logger=logger, name='target', dfs=dfs, **data.test.target).array
    ...
    if notice:
        ...
        logger.info(f"Downstream finished!")   
